    ...
    describe(BASE_ROUTE, () => {
        describe(`GET ${BASE_ROUTE}`, () => {
            ...
            it('should get services with pagination', async () => {
                ...
                const [{ body: body1 }, { body: body2 }, { body: body3 }] = await Promise.all([
                    request(app).get(BASE_ROUTE).set('Authorization', `Bearer ${viewerToken}`).query({ limit: 5, step: 0 }).expect(200),
                    request(app).get(BASE_ROUTE).set('Authorization', `Bearer ${viewerToken}`).query({ limit: 5, step: 1 }).expect(200),
                    request(app).get(BASE_ROUTE).set('Authorization', `Bearer ${viewerToken}`).query({ limit: 5, step: 2 }).expect(200),
                ]);
        ...
        describe(`PUT ${BASE_ROUTE}/:id`, () => {
            ...
            it('should reject parentId change that creates a cycle', async () => {
                const { body: a } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-A', parentId: null }).expect(200);
                const { body: b } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-B', parentId: a._id }).expect(200);
                const { body: c } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-C', parentId: b._id }).expect(200);
            ...
            it('should allow changing to a valid new parent (no cycle)', async () => {
                const { body: a } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-A', parentId: null }).expect(200);
                const { body: b } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-B', parentId: null }).expect(200);
                const { body: c } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-C', parentId: a._id }).expect(200);
        ...
        describe(`PATCH ${BASE_ROUTE}/:id/status`, () => {
            ...
            it('should cascade DOWN status up to the root', async () => {
                const { body: grandparent } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'GTA', parentId: null }).expect(200);
                const { body: parent } = await request(app)
                    .post(BASE_ROUTE)
                    .set('Authorization', `Bearer ${adminToken}`)
                ...
                const { body: parentAfter } = await request(app).get(`${BASE_ROUTE}/${parent._id}`).set('Authorization', `Bearer ${viewerToken}`).expect(200);
                const { body: grandparentAfter } = await request(app).get(`${BASE_ROUTE}/${grandparent._id}`).set('Authorization', `Bearer ${viewerToken}`).expect(200);
            ...
            it('should recover parent to UP only when all siblings are UP', async () => {
                const { body: parent } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-P', parentId: null }).expect(200);
                const { body: childA } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-A', parentId: parent._id }).expect(200);
                const { body: childB } = await request(app).post(BASE_ROUTE).set('Authorization', `Bearer ${adminToken}`).send({ name: 'SYSTEM-B', parentId: parent._id }).expect(200);

                await request(app).patch(`${BASE_ROUTE}/${childA._id}/status`).set('Authorization', `Bearer ${adminToken}`).send({ status: 'DOWN' }).expect(200);
                await request(app).patch(`${BASE_ROUTE}/${childB._id}/status`).set('Authorization', `Bearer ${adminToken}`).send({ status: 'DOWN' }).expect(200);

                await request(app).patch(`${BASE_ROUTE}/${childA._id}/status`).set('Authorization', `Bearer ${adminToken}`).send({ status: 'UP' }).expect(200);

                const { body: parentStillDown } = await request(app).get(`${BASE_ROUTE}/${parent._id}`).set('Authorization', `Bearer ${viewerToken}`).expect(200);
                expect(parentStillDown.status).toEqual('DOWN');

                await request(app).patch(`${BASE_ROUTE}/${childB._id}/status`).set('Authorization', `Bearer ${adminToken}`).send({ status: 'UP' }).expect(200);

                const { body: parentUp } = await request(app).get(`${BASE_ROUTE}/${parent._id}`).set('Authorization', `Bearer ${viewerToken}`).expect(200);
